...
public class SerialService extends Service implements SerialListener {
    ...
    private BluetoothDevice currentDevice; // Track connected device
    private byte[] lastData; // Store latest data
    ...
    public void connect(SerialSocket socket) throws IOException {
        Log.d(TAG, "Connecting to socket for device: " + (socket != null ? socket.getName() : "null"));
        socket.connect(this);
        this.socket = socket;
        connected = true;
    }
    ...
    public void disconnect() {
        Log.d(TAG, "Disconnecting, device: " + (currentDevice != null ? currentDevice.getAddress() : "none"));
        connected = false;
        cancelNotification();
        if (socket != null) {
            socket.disconnect();
        ...
        currentDevice = null;
        lastData = null; // Clear last data on disconnect
    }
    ...
    public void attach(SerialListener listener) {
        ...
        Log.d(TAG, "Attaching listener, device: " + (currentDevice != null ? currentDevice.getAddress() : "none"));
        initNotification();
        cancelNotification();
        synchronized (this) {
    ...
    public void detach() {
        Log.d(TAG, "Detaching listener, connected: " + connected);
        if (connected)
            createNotification();
        listener = null;
    }
    ...
    public void setCurrentDevice(BluetoothDevice device) {
        Log.d(TAG, "Setting current device: " + (device != null ? device.getAddress() : "null"));
        this.currentDevice = device;
    }

    public byte[] getLastData() {
        return lastData != null ? lastData : new byte[0];
    }
                ...
                if (listener != null) {
                    mainLooper.post(() -> {
                        if (listener != null) {
                            ...
                            setCurrentDevice(socket != null ? socket.getDevice() : null); // Set device on connect
                        } else {
    ...
    public void onSerialRead(ArrayDeque<byte[]> datas) {
        if (connected) {
            synchronized (this) {
                if (!datas.isEmpty()) {
                    lastData = datas.getLast(); // Store latest data
                    Log.d(TAG, "Received data, size: " + lastData.length);
                }
                if (listener != null) {
                    mainLooper.post(() -> {
                        if (listener != null) {
    ...
    public void onSerialRead(byte[] data) {
        if (connected) {
            synchronized (this) {
                ...
                Log.d(TAG, "Received data, size: " + data.length);
                if (listener != null) {
                    boolean first;
